...
if ( !class_exists( 'Fastshop_Shortcode_Container' ) ) {

	class Fastshop_Shortcode_Container extends Fastshop_Shortcode
	{
		/**
		 * Shortcode name.
		 *
		...
		public static function generate_css( $atts )
		{
			...
			if ( $content_width == 'custom_width' ) {
				$css = '.'.$container_custom_id.' { width: ' . $number_width . '; float: left; display: inline-block }';
			}
			return $css;
		}
		...
		{
			$atts = function_exists( 'vc_map_get_attributes' ) ? vc_map_get_attributes( 'fastshop_container', $atts ) : $atts;
			// Extract shortcode parameters.
			extract( $atts );

			$css_class   = array( 'fastshop-container' );
			...
			return apply_filters( 'Fastshop_Shortcode_container', force_balance_tags( $html ), $atts, $content );
		}
	}
}