	...
	class Ovic_Import_Dashboard
	{
		...
		public function admin_menu()
		{
			if ( current_user_can( 'edit_theme_options' ) ) {
				add_submenu_page( 'ovic-plugins', 'Ovic Import Demo', 'Ovic Import Demo', 'manage_options', 'ovic-import', array( $this, 'options_setting' ) );
			}
		}
                    ...
                    <div class="nav-tab-wrapper">
						<?php foreach ( $tabs as $key => $value ): ?>
                            <a class="nav-tab <?php if ( $tab == $key ): ?> nav-tab-active<?php endif; ?>"
                               href="<?php echo esc_url( 'admin.php?page=ovic-import&tab=' . $key ) ?>">
								<?php echo esc_html( $value ); ?>
                            </a>
						<?php endforeach; ?>
                    </div>
                    <div class="tab-content <?php echo esc_attr( strtolower( $tab ) ); ?>">
