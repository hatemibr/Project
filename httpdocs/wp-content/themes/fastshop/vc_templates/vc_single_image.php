...
$lazy_load_class = fastshop_get_option( 'fastshop_theme_lazy_load' ) == 1 ? 'lazy' : '';
$img             = false;
switch ( $source ) {
	case 'media_library':
	case 'featured_image':
		...
		$image_thumb      = '<img class="vc_single_image-img ' . $lazy_load_class . ' attachment-' . $img_size . '" src="' . $lazy_url . '" data-src="' . $image[0] . '" width="' . $image[1] . '" height="' . $image[2] . '" alt="' . get_the_title() . '"/>';
		$img              = array(
	...
	case 'external_link':
		...
		$img              = array(
			'thumbnail' => '<img class="vc_single_image-img ' . $lazy_load_class . '" ' . $hwstring . ' src="' . $lazy_url . '" data-src="' . $custom_src . '" alt="' . get_the_title() . '"/>',
		);
		break;
	default:
		$img = false;
